...
describe('CacheService (Integration)', () => {
  ...
  describe('scanAndDelete', () => {
    it('should delete all keys matching pattern', async () => {
      await service.set('scan:test:1', 'a');
      await service.set('scan:test:2', 'b');
      await service.set('scan:other:3', 'c');

      await service.scanAndDelete('scan:test:*');

      expect(await service.get('scan:test:1')).toBeNull();
      expect(await service.get('scan:test:2')).toBeNull();
      expect(await service.get('scan:other:3')).toBe('c');
    });

    it('should not throw when no keys match', async () => {
      await expect(
        service.scanAndDelete('nonexistent:*'),
